...
const suggestions = ['Radiohead', 'Fleetwood Mac', 'Tame Impala', 'The Strokes']
  ...
  <div>
    ...
    <section class="mx-auto grid max-w-5xl items-center gap-10 px-4 pb-8 pt-12 md:grid-cols-2 md:pt-20">
      ...
      <div class="relative mx-auto w-full max-w-sm">
        ...
        <div class="setlist-sheet -rotate-1 rounded-2xl border-2 border-espresso p-6">
          ...
          <ol class="mt-4">
            <li
              v-for="(t, i) in ['Golden Hour', 'Paper Moon', 'Reverb City', 'Cream Soda', 'Afterglow']"
              :key="t"
              class="flex items-baseline gap-3 py-1.5"
            >
              <span class="w-6 text-right font-mono text-sm text-burnt">{{ String(i + 1).padStart(2, '0') }}</span>
              <span class="font-medium text-espresso">{{ t }}</span>
            </li>
          </ol>
          <div class="mt-4 flex items-center gap-2 rounded-full border-2 border-espresso bg-burnt px-4 py-2 font-display font-semibold text-paper">
    ...
    <section id="how" class="scroll-mt-20 bg-paper py-16">
      <div class="mx-auto max-w-5xl px-4">
        <h2 class="text-center font-display text-3xl font-bold text-espresso sm:text-4xl">Three steps, one souvenir</h2>
        <ol class="mt-10 grid gap-6 md:grid-cols-3">
          <li v-for="(step, i) in steps" :key="step.title">
            <UiCard tone="cream" class="h-full">
              <div class="p-6">
                <div class="flex items-center gap-3">
                  <span class="font-display text-4xl font-bold text-mustard">{{ String(i + 1).padStart(2, '0') }}</span>
                  <span class="grid h-11 w-11 place-items-center rounded-full border-2 border-espresso bg-cream">
                    <Icon :name="step.icon" size="22" class="text-espresso" />
                  </span>
                </div>
                <h3 class="mt-4 font-display text-xl font-semibold text-espresso">{{ step.title }}</h3>
    ...
    <section class="mx-auto max-w-3xl px-4 py-16 text-center">
      <h2 class="font-display text-3xl font-bold text-espresso sm:text-4xl">Which show do you miss?</h2>
