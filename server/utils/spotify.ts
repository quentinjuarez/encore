...
async function refreshAccessToken(
  ...
  const { clientId, clientSecret } = (
    useRuntimeConfig(event) as unknown as {
  ...
  try {
    ...
    if (status === 400 || status === 401) {
      ...
      throw createError({
        statusCode: 401,
        statusMessage: 'Spotify session expired, please reconnect',
      });
    }
...
function spotifyError(err: unknown, path: string, grantedScope?: string) {
  ...
  console.error(
    `[spotify] ${path} failed (${status}): ${reason} | granted scope: ${grantedScope ?? 'unknown'}`,
  );
  return createError({
    statusCode: status,
